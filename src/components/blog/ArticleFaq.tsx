...
export const ArticleFaq: React.FC<ArticleFaqProps> = ({ faqs, articleTitle }) => {
  const [openIndex, setOpenIndex] = useState<number | null>(0);

  if (!faqs || faqs.length === 0) return null;

  const whatsappUrl = `https://wa.me/${siteConfig.whatsappClean}?text=${encodeURIComponent(
    `Olá! Estava lendo o artigo "${articleTitle}" no blog da EasyTraining e gostaria de tirar uma dúvida.`
  )}`;

  return (
    <section aria-labelledby="article-faq-heading" className="my-10">
      <div className="bg-white rounded-3xl p-6 sm:p-10 shadow-sm border border-slate-200/80 space-y-8">
        <div className="flex items-start gap-3.5 border-b border-slate-100 pb-6">
          ...
          <div>
            ...
            <h2 id="article-faq-heading" className="text-2xl sm:text-3xl font-black text-[#052e7f] tracking-tight">
              Perguntas Frequentes sobre este tema
            </h2>
            <p className="text-xs sm:text-sm text-slate-500 mt-1">
              Respostas rápidas para complementar a leitura e ajudar você a tomar sua decisão.
            </p>
          </div>
        </div>

      <div className="space-y-3.5" role="region" aria-label="Lista de perguntas frequentes do artigo">
        {faqs.map((faq, index) => {
          const isOpen = openIndex === index;
          const questionId = `article-faq-question-${index}`;
          const answerId = `article-faq-answer-${index}`;
          ...
          return (
            ...
            >
              ...
              >
                <span className="text-sm sm:text-base font-bold text-slate-800 leading-snug">{faq.question}</span>
              ...
              {isOpen && (
                ...
                >
                  <div className="border-t border-slate-200/60 pt-3">{faq.answer}</div>
                </div>
              )}
            </div>
          );
      ...
      <div className="mt-8 p-5 sm:p-6 rounded-2xl bg-gradient-to-br from-slate-900 via-slate-800 to-[#052e7f] text-white flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm">
        <div className="space-y-1 text-center sm:text-left">
          <h3 className="text-sm sm:text-base font-bold">Ainda ficou com alguma dúvida?</h3>
          <p className="text-xs text-slate-300">Nossa equipe da EasyTraining está pronta para ajudar.</p>
        </div>
        <a
          href={whatsappUrl}
          target="_blank"
